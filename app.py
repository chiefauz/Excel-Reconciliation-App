...
def parse_statement_summary(raw: pd.DataFrame) -> dict:
    ...
    for _, row in raw.head(10).iterrows():
        rowtxt = " | ".join([str(x) for x in row.tolist() if pd.notna(x)])
        if "Bank Charges" in rowtxt:
            nums = re.findall(r"\d+(?:\.\d+)?", rowtxt)
            bank_charges = float(nums[-1]) if nums else 0.0
        if "Gross Sales" in rowtxt:
            nums = re.findall(r"\d+(?:\.\d+)?", rowtxt)
            gross_sales = float(nums[-1]) if nums else 0.0
        if "Value Added Taxes" in rowtxt or "VAT" in rowtxt:
            nums = re.findall(r"\d+(?:\.\d+)?", rowtxt)
            vat = float(nums[-1]) if nums else 0.0
    return {
...
def load_reconciliation(uploaded_file) -> pd.DataFrame:
    ...
    # This workbook is sparse/merged, so fixed column positions are more reliable.
    df = raw.iloc[7:].copy()
    df = df[df[1].notna()].copy()
    df["DateTime"] = pd.to_datetime(df[1], errors="coerce")
    ...
    out = pd.DataFrame({
        ...
        "Seq": df[3].astype(str),
        "Node": df[7].astype(str),
        "User": df[11].astype(str),
        "Amount": df[20].map(parse_amount_text),
        "Amount_Raw": df[20].astype(str),
        "Card_Brand": df[27].map(normalize_brand),
        "Last4": df[32].map(extract_last4),
        "Auth_Code": df[40].astype(str),
        "Auth_Norm": df[40].map(normalize_auth),
        "Authorization_Status": df[42].astype(str),
        "Processor_Ref": df[38].astype(str),
        "Response": df[47].fillna("").astype(str),
    })
    ...
    out["Is_Decline"] = status_text.str.contains("DECLIN|DENIED|ERROR|FAILED", na=False)
    out["Duplicate_Key"] = (
...
def load_statement(uploaded_file):
    ...
    df["Purchase_Date"] = pd.to_datetime(df[0], errors="coerce")
    df = df[df["Purchase_Date"].notna()].copy()

    out = pd.DataFrame({
        "Source_Row": df.index + 1,
        "Purchase_Date": df["Purchase_Date"].dt.date,
        "Card_Brand": df[1].map(normalize_brand),
        "Last4": df[3].map(extract_last4),
        "Card_Number_Masked": df[3].astype(str),
        "Auth_Code": df[4].astype(str),
        "Auth_Norm": df[4].map(normalize_auth),
        "Amount": pd.to_numeric(df[9], errors="coerce"),
        "Trans_Type": df[7].astype(str).str.strip(),
    })

    out = out[out["Amount"].notna()].copy()
    out = out[out["Trans_Type"].str.upper().eq("PURCHASE")].copy()
...
def create_excel_report(rec, stmt, merchant_summary, matched, rec_unmatched, stmt_unmatched):
    ...
    summary_rows = [
        ...
        ["Next-day matches", summary.next_day_count, summary.next_day_amount, None, "Stmt unmatched same date", f'{summary.stmt_unmatched_same_date_count} / {summary.stmt_unmatched_same_date_amount:,.2f}'],
        [None, None, None, None, "Stmt unmatched other date", f'{summary.stmt_unmatched_other_date_count} / {summary.stmt_unmatched_other_date_amount:,.2f}'],
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        for ws in wb.worksheets:
            ...
            if ws.title in {"Summary", "Statement_Summary", "Exceptions"}:
                for cell in ws[1]:
                    cell.font = cell.font.copy(bold=True)
            if ws.title == "Matched" and ws.max_row > 1:
                ws.freeze_panes = "A2"
