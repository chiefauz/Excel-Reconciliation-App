...
APP_TITLE = "Excel Reconciliation App v6"
...
# -----------------------------
# Reconciliation parsing by logical visible columns
# -----------------------------
...
def load_reconciliation(uploaded_file):
    ...
    logical_headers = [normalize_text(v) for _, v in header_cells]

    expected = ["date/time", "seq", "node", "user", "payment/refund total", "card type", "card number"]
    if not all(any(e in h for h in logical_headers) for e in expected):
    ...
    for ridx in range(header_row + 1, len(raw)):
        ...
        # logical layout based on actual export:
        # 0 datetime, 1 seq, 2 node, 3 user, 4 amount, 5 brand, 6 card, 7 processor ref, 8 auth, 9 status
        ...
        if pd.isna(dt) or pd.isna(amount):
            continue
    ...
    if out.empty:
        raise ValueError("No reconciliation transactions were parsed.")
    ...
    meta = {"header_row": header_row + 1, "mapping": mapping}
...
def load_statement(uploaded_file):
    ...
    meta = {"header_row": header_row + 1, "mapping": {k: f"{excel_col(v)} ({v})" if v is not None else None for k, v in mapping_idx.items()}}
...
def create_excel_report(rec, stmt, merchant_summary, matched, rec_unmatched, stmt_unmatched, rec_meta, stmt_meta):
    ...
    summary_rows = [
        ...
        ["Generated by Streamlit app v6", None, None, None, None, None],
...
def main():
    ...
    st.title(APP_TITLE)
    st.write("Version 6 uses visible-column parsing for the reconciliation workbook and preserves the Feb 9 matching logic.")
    ...
    if rec_file and stmt_file:
        try:
            ...
            with st.expander("Detected reconciliation mapping"):
                st.write(rec_meta)
            with st.expander("Detected merchant mapping"):
                st.write(stmt_meta)

            st.subheader("Reconciliation parsed rows")
            st.dataframe(rec.head(50), use_container_width=True)
